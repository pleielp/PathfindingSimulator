...
class Simulator(object):
    ...
    def handle_event(self):
        ...
        for event in pygame.event.get():
            ...
            # 스페이스 바
            elif e_type == constants.KEY_UP and e_dict["key"] == constants.KEY_SPACE:
            ...
            # 대기상태 일 때 ESC
                ...
                self.status == "wait"
                and e_type == constants.KEY_UP
                and e_dict["key"] == constants.KEY_ESCAPE
            ):
                for cell in self.cells_flatten:
            ...
            # 대기상태 일 때 마우스 입력
            elif self.status == "wait" and e_type in {
                constants.MOUSE_BUTTON_DOWN,
                constants.MOUSE_MOTION,
                constants.MOUSE_BUTTON_UP,
            }:
                x, y = e_dict["pos"]
            ...
            # 일시정지 중이거나 완료됐을 때
                ...
                self.status in ("pause", "complete")
                and e_type in constants.MOUSE_EVENTS
            ):
                x, y = e_dict["pos"]
                x_idx, y_idx = x // self.cell_size, y // self.cell_size
                self.debug_list.clear()
    ...
    def run(self):
        ...
        for cell in self.prev_cells - self.next_cells:
            cell.color = constants.COLOR_LIGHT_BLUE
        # 자료구조에 추가된(확인할) cell 은 초록 색으로 색칠
        for cell in self.next_cells - self.prev_cells:
            cell.color = constants.COLOR_LIGHT_GREEN
    ...
    def complete(self):
        ...
        if self.start_time:  # 알고리즘 실행 끝난 후 한 번만 통과
            ...
            self.path = [
                tuple(map(self.resize_idx_to_cell, (idx_x, idx_y)))
